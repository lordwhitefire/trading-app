...
export default function TradePage() {
    ...
    // ── Poll live prices from Render device endpoint (phone → Render) ─────────
    useEffect(() => {
        if (!openTrades.length) return;
        
        const fetchPricesFromDevice = async () => {
            try {
                const res = await fetch(`${API}/api/device/prices`);
                const data = await res.json();
                const devicePrices = data.prices || {};
                
                // Map prices to trade IDs
                const byId: Record<string, number> = {};
                openTrades.forEach((trade: any) => {
                    const coin = trade.coin;
                    if (devicePrices[coin] !== undefined) {
                        byId[trade.id] = devicePrices[coin];
                    }
                });
                setLivePrices(byId);
            } catch (err) {
                console.error('Failed to fetch prices from device endpoint:', err);
            }
        };
        
        fetchPricesFromDevice();
        const interval = setInterval(fetchPricesFromDevice, 10000); // every 10 seconds
